...
public class TrackArrayY : MonoBehaviour {
	...
	public void BarCheck(){
		...
		var target = new Vector2(headScript.rdHeadPos.x,headScript.rdHeadPos.z);
	...
	void FoodCheck(){
		for(int i = 0; i < Bars.Count; i++){
			...
			((Bars[i].transform.localPosition.y + .5 == Food.myPos.z)||(Bars[i].transform.localPosition.y - .5 == Food.myPos.z)))||
			((Bars[i].transform.localPosition.y == Food.myPos.z) &
