...
public class TrackArrayX : MonoBehaviour {
	...
	public void BarCheck(){
		...
		var target = new Vector2(-1*(headScript.rdHeadPos.z),headScript.rdHeadPos.y);
	...
	void FoodCheck(){
	for(int i = 0; i < Bars.Count; i++){
		if(((Bars[i].transform.localPosition.x == -1*(Food.myPos.z)) &
		...
		((Bars[i].transform.localPosition.x + .5 == -1*(Food.myPos.z))||(Bars[i].transform.localPosition.x - .5 == -1*(Food.myPos.z))))){
			isFood[i] = true;
		}else{
			isFood[i] = false;
		}
